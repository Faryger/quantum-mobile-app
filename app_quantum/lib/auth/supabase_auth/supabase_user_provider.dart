...
class AppQuantumSupabaseUser extends BaseAuthUser {
  AppQuantumSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> appQuantumSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AppQuantumSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
